...
namespace GloriousGamesEcommerce.Services.ShoppingCartAPI.Controllers
{
    [ApiController]
    [Route("api/cart")]
    public class CartAPIController : Controller
    {
        ...
        public async Task<object> GetCart(string userId)
        {
            try
            {
                CartDto cartDto = await _cartRepository.GetCarByUserId(userId);
        ...
        public async Task<object> Checkout(CheckoutHeaderDto checkoutHeader)
        {
            try
            {
                CartDto cartDto = await _cartRepository.GetCarByUserId(checkoutHeader.UserId);
                if (cartDto == null)
                {
                    return BadRequest();
                }
                ...
                if (!string.IsNullOrEmpty(checkoutHeader.CouponCode))
                {
                   CouponDto coupon = await _couponRepository.GetCoupon(checkoutHeader.CouponCode);
                    ...
                    {
                        _response.IsSuccess = false;
                        _response.ErrorMessages = new List<string>() { "Coupon price has changed, please try a new code" };
                        _response.DisplayMessage = "Coupon price has changed, please try a new code";
                        return _response;
                    }
                }
